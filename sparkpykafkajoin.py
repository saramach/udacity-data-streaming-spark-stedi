...
spark = SparkSession.builder.appName("join-fun").getOrCreate()
...
zSetDecodedEntriesStreamingDF.withColumn("customer", from_json("customer", customerSchema)).select(col('customer.*')).createOrReplaceTempView("CustomerRecords")
...
# Be sure to specify the option that reads all the events from the topic including those that were published before you started the spark stream
stediRiskEventsRawStreamingDF = spark \
    .readStream \
    .format("kafka") \
    .option("kafka.bootstrap.servers", "localhost:9092") \
...
# TO-DO: cast the value column in the streaming dataframe as a STRING 
stediRiskEventsStreamingDF = stediRiskEventsRawStreamingDF.selectExpr("cast(value as string) value")
...
costumerRiskWithBirthYearStreamingDF = customerRiskStreamingDF.join(emailAndBirthYearStreamingDF, 
                                                                    expr("""
                                                                       customer=email
                                                                    """))
